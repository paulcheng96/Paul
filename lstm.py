...
def loadfile():
    ...
    neg = rm_stopwords(neg[0])
    pos = rm_stopwords(pos[0])
    neu = rm_stopwords(neu[0])
...
def train_lstm(n_symbols,embedding_weights,x_train,y_train,x_test,y_test):
    ...
    model.add(LSTM(output_dim=100, activation='tanh', return_sequences=True))
    model.add(LSTM(output_dim=100, activation='tanh'))
    model.add(Dropout(0.3))
    model.add(Dense(3, activation = 'sigmoid')) # Dense=>全连接层,输出维度=3
